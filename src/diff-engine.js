...
export function generateHtmlDiff(diffOps, mode = "word") {
  ...
  diffOps.forEach(op => {
    switch (op.type) {
      case "same":
        leftHtml.push(`<span class="diff-same">${escapeHtml(op.text)}</span>`);
        rightHtml.push(`<span class="diff-same">${escapeHtml(op.text)}</span>`);
        break;
      case "del":
        leftHtml.push(`<span class="diff-del">${escapeHtml(op.text)}</span>`);
        break;
      case "add":
        rightHtml.push(`<span class="diff-add">${escapeHtml(op.text)}</span>`);
        break;
    }
  });
